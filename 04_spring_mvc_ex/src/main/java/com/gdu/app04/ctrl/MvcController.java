...
public class MvcController {
  ...
  // DispatcherServlet(servlet-context.xml)에서 ViewResolver를 제거했으므로 JSP의 전체 경로를 작성해야한다. 
  
  @RequestMapping(value="/", method = RequestMethod.GET)  //"/"만 입력하면 contextPath다 입력되는 것(http://localhost:8080/app03/)
  public String main() {
 ...
 //@RequestMapping(value="/register.do", method=RequestMethod.POST)
  ...
  //@RequestMapping(value="/register.do", method=RequestMethod.POST)
  public String register3(ArticleVo vo) { //내부적으로는 setter 사용됨.
    //모델의 선언이 불필요 이미 모델에 저장이 되어 있음 ArticleVo로 이미 저장이 됨
    return "/WEB-INF/article/result.jsp"; 
  }
  @RequestMapping(value="/register.do", method=RequestMethod.POST)
