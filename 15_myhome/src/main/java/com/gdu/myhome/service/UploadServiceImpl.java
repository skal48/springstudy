...
public class UploadServiceImpl implements UploadService {
  ...
  public boolean addUpload(MultipartHttpServletRequest multipartRequest) throws Exception {
    ...
    for(MultipartFile multipartFile : files) {
      ...
      if(multipartFile != null && !multipartFile.isEmpty()) {
        ...
        String contentType = Files.probeContentType(file.toPath()); //이미지의 content-Type : image/jpeg, image/png 등 image 로 시작한다. 
        int hasThumbnail = (contentType != null && contentType.startsWith("image")) ? 1 : 0;
        
        if(hasThumbnail == 1) {
          File thumbnail = new File(dir, "s_" + filesystemName);  //small 이미지를 의미하는 s를 붙인다. 
          Thumbnails.of(file)
                    .size(100, 100)   //가로 세로 100px
                    .toFile(thumbnail);
        }
    ...
    return (addUploadResult == 1) && (files.size() == attachCount);
  ...
  @Transactional(readOnly = true)
  @Override
  public UploadDto getUpload(int uplaodNo) {   
    return uploadMapper.getUpload(uplaodNo);
  }
