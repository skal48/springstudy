...
<jsp:include page="../layout/header.jsp">
  <jsp:param value="${blog.blogNo}블로그" name="title"/>
</jsp:include>
<style>
 ...
 <div>
  ...
  <div>
    <form id="frm_comment_add">
      ...
      <button type="button" id="btn_comment_add">작성다하면클릭!!</button>
    </form> 
    <script>
    const fnRequiresLogin = () => {
    	//로그인인을 안하고 작성을 시도하면 로그인 페이지 보내기  
    ...
    const fnCommentList = () => {
        $.ajax({
          ...
          success: (resData) => {  // resData = {"commentList": [], "paging": "<div>...</div>"}
            if(resData.commentList === 0){
            	$('#comment_list').text('첫번째 댓글의 주인공이 되어 보세요');
            	$('#paging').text('');
            	return;
            }
