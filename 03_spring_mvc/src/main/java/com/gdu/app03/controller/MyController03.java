...
public class MyController03 {
  ...
  *   1) Java EE 표준 방식이다. 
  *   2) 파라미터뿐만 아니라 session, contextPath 와 같은 정보로도 꺼낼 수 있으므로 여전히 강력한 도구이다 .
  */
  
  
  //@RequestMapping("/blog/detail.do") //GET 방식의 method는 생략할 수 있다. value 만 작성할 땐 value= 부분도 생략 할 수 있다. 
  public String blogDetail(HttpServletRequest request, Model model) {
    ...
    return "blog/detail";  //WEB-INF/views/blog/detail.jsp로 forward 한다. 
  }
  ...
  /*
   * 3. 커맨드 객체를 이용항 요청 파라미터 처리 
   *  1) 요청 파라미터를 필드로 가지고 있는 객체를 커맨드 객체라고 한다. 
   *  2) 요청 파라미터를 필드에 저장할 때 Setter가 사용된다. 
   *  3) 요청 파라미터가 많은 경우에 유용하다.  
   *  4) 커맨드 객체는 자동으로 Model에 저장된다. 저장될때 객체명(dto)이 아닌 클래스명(BlogDto)으로 저장된다. (클래스 명을 LowerCamelCase로 바꿔서 저장한다.)
   */
